...
class StudentsForResponsibleSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!$stase) {
            $this->command->error('Stase untuk user ID 3 tidak ditemukan. Gunakan ResponsibleDataSeeder terlebih dahulu.');
            return;
        }
    ...
    private function createScheduleIfNeeded($stase, $internshipClass)
    {
        ...
        $existingOverlap = Schedule::where('stase_id', $stase->id)
            ...
            ->where(function($query) use ($startDate, $endDate) {
                // Cek jika ada overlap dengan range yang ada
        ...
        if (!$existingOverlap) {
            ...
            $this->command->info('Jadwal sudah ada untuk stase ' . $stase->name . ' dan kelas ' . $internshipClass->name);
        }
    }
