@foreach ($studyPrograms as $i => $studyProgram)
    <tr class="hover:bg-gray-50">
        ...
        <td class="px-6 py-4 flex space-x-2 text-center whitespace-nowrap text-sm font-medium">
            ...
            <form id="delete-student-{{ $studyProgram->id }}"
                action="{{ route('admin.studyPrograms.destroy', $studyProgram->id) }}" method="POST"
                onsubmit="return confirm('Apakah Anda yakin ingin menghapus Study Program ini?');">
                @csrf
                @method('DELETE')
