...
class AdminScheduleController extends Controller
{
    ...
    {
        $schedules = Schedule::with(['internshipClass', 'departement', 'responsibleUser'])
            ->paginate(10);
        $departments = Departement::all();
        $responsibles = ResponsibleUser::with('user_id')->get();
    ...
    public function filter(Request $request)
    {
        ...
        if ($request->has('year') && $request->year != '') {
            $query->whereHas('internshipClass.classYear', function($q) use ($request) {
                $q->where('class_year', $request->year);
            });
        }
        ...
        $schedules = $query->with(['internshipClass', 'departement', 'responsibleUser.user'])->paginate(10);
